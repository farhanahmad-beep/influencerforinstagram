...
const UserDiscovery = () => {
  ...
  // Toggle between Global Search and Followers modes
  const [activeMode, setActiveMode] = useState("global-search"); // "global-search" or "followers"
...
Let me know if you want help getting started! 😊`;
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (accounts.length > 0) {
          ...
          setFilters(prev => ({ ...prev, account_id: accounts[0].id }));
        }
      }
    } catch (error) {
  ...
  const handleViewProfile = (userId) => {
    ...
    navigate(`/user-profile/${userId}`, {
      state: { accountId: selectedAccountId, from: activeMode === "global-search" ? "global-search" : "followers" },
    });
  };
  ...
  const handleSelectAll = () => {
    ...
    const usersWithMessagingId = data
        ...
        if (response.data.success) {
          ...
          try {
            await axios.post(`${import.meta.env.VITE_API_URL}/influencers/user-status/contacted`, {
              ...
              source: activeMode === "global-search" ? 'global_search' : viewMode,
            }, { withCredentials: true });
          } catch (statusError) {
  ...
  const handleDownloadCSV = () => {
    ...
    try {
      ...
      if (link.download !== undefined) {
        ...
        link.setAttribute('download', `${activeMode === "global-search" ? 'global_search' : viewMode}_results_${new Date().toISOString().split('T')[0]}.csv`);
      ...
      toast.success(`Downloaded ${data.length} results to CSV`);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:pt-4 pt-16">
          <div className="mb-8">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h1 className="text-3xl font-bold text-gray-900 mb-2">User Discovery</h1>
                <p className="text-gray-600">
                  {activeMode === "global-search"
                    ? "Search Instagram users globally without providing account_id."
                    : "View and manage followers and following accounts from your linked Instagram accounts"}
                </p>
              </div>
            ...
            <form onSubmit={handleSearch} className="space-y-4">
              ...
              <div className="flex items-center space-x-4">
                <button type="submit" className="btn-primary" disabled={loading}>
                  {loading ? "Loading..." : activeMode === "global-search" ? "Search Users" : "Search" + (viewMode === "following" ? " Following" : " Followers")}
                </button>
                <p className="text-sm text-gray-500">
                  {activeMode === "global-search"
                    ? "Powered by Rocket API"
                    : filters.account_id
                      ? "✓ Ready to search"
          ...
          {/* Results */}
          {loading && (activeMode === "global-search" ? results : allData).length === 0 ? (
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="card text-center py-12">
              ...
              <p className="text-gray-500">
                {activeMode === "global-search"
                  ? "Try another keyword or adjust follower filters."
                  : "Enter a username or account ID to search for followers/following."}
              </p>
            </motion.div>
          ) : (
            <>
              <div className="mb-4 flex items-center justify-between">
                <p className="text-sm text-gray-600">
                  ...
                  <span className="font-semibold">{activeMode === "global-search" ? results.length : allData.length}</span> results
                  ...
                  {isSelectionMode && (
                    <>
                      ...
                      >
                        {(() => {
                          const data = filteredResults;
                          const usersWithMessagingId = data.filter((item) => item.providerMessagingId).map((item) => item.id);
                    ...
                    <div className="flex flex-col items-center text-center">
                      ...
                      <div className="w-full">
                        <div className="flex items-center justify-center space-x-2 mb-1">
                          ...
                          {item.isFavorite && activeMode === "followers" && viewMode === "following" && (
